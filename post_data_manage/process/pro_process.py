...
def drop_unnormal(df):
    ...
    # 确定需要剔除的快递员
    rmv_wd = set(filter(lambda c: work_days[c] < 5, couriers)) #工作少于5天的快递员数据不要
    rmv_dmd = set(filter(lambda c: dis_mean_day[c] < 50, couriers)) #每日平均移动距离少于50m的不要
    rmv_omd = set(filter(lambda c: order_mean_day[c] < 5, couriers)) # 每日平均单数少于3单的不要
    rmv_tmo = set(filter(lambda c: time_mean_order[c] < 5, couriers)) # 每两单之间的时间小于3min的不要
    rmv_dmo = set(filter(lambda c: dis_mean_order[c] < 50, couriers)) # 每两单之间的平均距离少于20m的不要
...
def courier_info(df):
    ...
    for courier in couriers:
        ...
        time_mean_order[courier]=np.mean(df[df['快递员id']==courier]['时间差单位化'])
        dis_mean_order[courier] = np.mean(df[df['快递员id']==courier]['与前一单直线距离'])
...
def distance(fro,to,df_v):
    ...
    if (str(fro)+'-'+str(to)) in dic_dis_cal:
        ...
        dis_temp=int(geodesic((df_v[fro-1][11],df_v[fro-1][10]),(df_v[to-1][11],df_v[to-1][10])).meters)
...
def pre_process( is_test=False,data_df=None):
    ...
    for c in courier_l:
        ...
        for n in range(c.shape[0]):
            ...
            dis_dir.append(int(geodesic((lat_, lon_), (c_v[n][8], c_v[n][7])).meters))
            got_time.append(t1)
    ...
    ##获取快递员统计信息
    # couriers, work_days, dis_mean_day, order_mean_day, time_mean_order, dis_mean_order = courier_info(df)
    df.insert(df.shape[1] - 2, '快递员平均距离', [dis_mean_day[c] for c  in df['快递员id']])
    df.insert(df.shape[1] - 2, '与前一单相对距离', [0 for n in range(df.shape[0])])
    df.insert(df.shape[1] - 2, '快递员平均间隔时间', [time_mean_order[c] for c in df['快递员id']])
    ...
    ##生成简化订单数据
    df_sim = df[['order_id', '日期', '揽收时间1', '接单时间1', '订单经度', '订单纬度', '时间差单位化', '与前一单相对距离']]
    # 重新编码，避免非数字
